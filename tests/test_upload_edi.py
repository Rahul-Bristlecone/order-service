...
def test_upload_edi_creates_order(client, app, monkeypatch):
    ...
    monkeypatch.setattr(
        "order_service.resources.orders.transform_edifact_to_json",
...
def test_upload_edi_rejects_duplicate_business_key(client, app, monkeypatch):
    ...
    monkeypatch.setattr("order_service.resources.orders.transform_edifact_to_json", lambda *_args, **_kwargs: payload)
...
def test_reject_order_uses_cached_draft_for_minimal_payload(client, app, monkeypatch):
    ...
    store = {}
    monkeypatch.setattr("order_service.resources.orders.redis_client.setex", lambda key, ttl, value: store.__setitem__(key, value))
    ...
    monkeypatch.setattr("order_service.resources.orders.redis_client.get", lambda key: '{"token": "%s"}' % token if key == "session:7" else store.get(key))
